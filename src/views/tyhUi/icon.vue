...
const { iconList, configures, iconLength } = iconOptions()
function iconOptions () {
  ...
  const copy = icon => {
    ...
    clipboard.on('success', _ => {
      Message({ message: `复制成功 ${icon}`, type: 'success', iconClass: 'tyh-ui-smile', round: true })
      clipboard.destroy()
    })
  }

  return { iconList, configures, iconLength }
}
</script>
